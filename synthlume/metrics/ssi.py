...
class SentenceSeparabilityIndex(Metric):
    ...
    def _train_linear_regression(self, x_true: list[list[float]], x_gen: list[list[float]]) -> None:
        logger.debug(f"Training linear regression model on {len(x_gen) + len(x_true)} samples")
